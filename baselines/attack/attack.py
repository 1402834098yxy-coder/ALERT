...
class Attacker:
    ...
    def __init__(self,sim_kw_d,real_td_d,sim_F=None,real_F=None,\
        ...
        # end_time = time.time()
        # print(f"Time taken for matrix operations: {end_time - start_time} seconds")
    ...
    def attack_step_3(self):
        ...
        while(len(self.unrec_td_set)>0):
            ...
            epsilon = 1e-10  # 或其他小的正数
            # M = M/M.sum(axis = 1).reshape((len(M),1))
            # M_ = M_/M_.sum(axis = 1).reshape((len(M_),1))
            M = M / (M.sum(axis=1).reshape((len(M),1)) + epsilon)
            M_ = M_ / (M_.sum(axis=1).reshape((len(M_),1)) + epsilon)
            ...
            for i in range(len(top_td)):
                epsilon = 1e-10  
                kw = np.argmax(-np.log(np.linalg.norm(M[top_td[i]]-M_,axis=1) + epsilon))
    ...
    def RSA(self):
        ...
        while(len(self.unrec_td_set)>0):
            ...
            for i in range(len(M)):
                epsilon = 1e-10  
                score = -np.log(np.linalg.norm(M[i]-M_,axis=1)+epsilon)
                score = sorted(score,reverse=True)
                certainty = score[0]-score[1]
                Certainty.append([i,certainty])
            Certainty.sort(key = lambda x:x[1],reverse=True)
