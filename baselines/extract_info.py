...
def get_F(kws_list,kws_dict,time):
    ...
    sum_F = np.sum(kws_F)
    if sum_F == 0:
        kws_F = np.ones_like(kws_F) / len(kws_F)
    else:
        kws_F = kws_F / sum_F
    
    kws_F = np.nan_to_num(kws_F)
...
def get_all_for_attacks_wiki(
    ...
    if dataset == "wiki":
        ...
        if max_size <= 3000:
            ...
            kw_dir = "dataset/kws_dict_7000_sorted.pkl"
            db_dir = "dataset/kws_list_and_doc_kws_new_7000_0.pkl"
        # elif max_size <= 10000:
