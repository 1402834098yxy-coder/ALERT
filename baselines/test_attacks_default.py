...
import os
import argparse
def run_Jigsaw_IHOP_and_RSA_default(test_times=30,kws_uni_size=1000,\
    ...
    if not os.path.exists("./result"):
        os.makedirs("./result")
    if not os.path.exists("./result/test_attacks_default"):
        os.makedirs("./result/test_attacks_default")
    print("Test Jigsaw IHOP and RSA under default")
    ...
    for dataset in datasets:
        ...
        for i in tqdm(range(test_times)):
            ...
            print({"Jigsaw:  dataset":dataset, "attack params":our_attack_params,"acc":acc})
            Jigsaw_Result.append((dataset,our_attack_params,acc,result))
...
################RSA##############
            ...
            correct_count,acc,correct_id,wrong_id = \
                calculate_acc_weighted(data_for_acc_cal,result["results"][0])
            print({"RSA:   dataset":dataset, "attack params":rsa_attack_params,"acc":acc})
            RSA_Result.append((dataset,rsa_attack_params,acc,result))
...
################IHOP#################################
            ...
            print({"IHOP:   dataset":dataset, "attack params":ihop_attack_params,"acc":acc})
            IHOP_Result.append((dataset,ihop_attack_params,acc,result))
...
if __name__ == "__main__":
    ...
    run_Jigsaw_IHOP_and_RSA_default(
