...
def generate_result(dataset,kws_uni_size):
    with open(f"result/test_attacks_low_latency/Re_{dataset}_{kws_uni_size}.json","r") as f:
        data = json.load(f)
        ...
        for i in range(len(desired_times)):
            for key in acc_keys:
                ...
                average_runtime = data[time_keys[j]][i]

                print(f"\n\\addplot[only marks, mark=star, color=black] coordinates {{")
                print(f"    (1.5, {average_runtime})")
                print(f"}};")

                with open(f"final_result/test_attacks_low_latency/{method_name}_{dataset}_{kws_uni_size}_{desired_times[i]}.txt", 'w') as f:
