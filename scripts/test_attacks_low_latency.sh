...
for i in "${!BETA_TEST_LIST[@]}"; do
    BETA_TEST="${BETA_TEST_LIST[$i]}"
    DESIRED_TIME="${DESIRED_TIMES[$i]}"
    
    echo "Running test with BETA_TEST: $BETA_TEST (Desired Time: $DESIRED_TIME)"
...
do
    ...
    LOG_TRAIN_FILE="${LOG_BASE_DIR}ALERT_${DATASET}_${EXPERIMENT_SETTING}_${candidate}_${COUNTERMEASURES}_${FACTOR}_${FPBP_DEL_PERCENTAGE}_${DESIRED_TIME}_${run}_train.log"
    LOG_TEST_FILE="${LOG_BASE_DIR}ALERT_${DATASET}_${EXPERIMENT_SETTING}_${candidate}_${COUNTERMEASURES}_${FACTOR}_${FPBP_DEL_PERCENTAGE}_${DESIRED_TIME}_${run}_test.log"
...
for FPBP_DEL_PERCENTAGE in "${FPBP_DEL_PERCENTAGES[@]}"
do
echo "FPBP percentage: $FPBP_DEL_PERCENTAGE"

  LOG_BASE_FILE="${PROJECT_ROOT}/result/${DATASET}/${EXPERIMENT_SETTING}/ALERT_${DATASET}_${EXPERIMENT_SETTING}_${candidate}_${COUNTERMEASURES}_${FACTOR}_${FPBP_DEL_PERCENTAGE}_${DESIRED_TIME}_"


        
PART='test'
OUTPUT_DIR="${OUTPUT_folder}/ALERT_${DATASET}_${EXPERIMENT_SETTING}_${DESIRED_TIME}_${PART}_"
# mkdir -p $OUTPUT_DIR
...
for dataset in "${DATASETS[@]}"; do
    python ${PROJECT_ROOT}/baselines/test_attacks_low_latency.py \
        --dataset "$dataset" \
        --test_times "$RUN_TIME" \
        --kws_uni_size "$FEATURE_NUM" \
        --kws_extraction "$KWS_EXTRACTION"

    python ${PROJECT_ROOT}/baselines/generate_test_attacks_low_latency.py \
        --dataset "$dataset" \
        --test_times "$RUN_TIME" \
        --kws_uni_size "$FEATURE_NUM" \
        --kws_extraction "$KWS_EXTRACTION"
    python ${PROJECT_ROOT}/baselines/generate_result_low_latency.py \
        --dataset "$dataset" \
        --kws_uni_size "$FEATURE_NUM"
done
