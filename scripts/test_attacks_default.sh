...
do
    ...
    LOG_TRAIN_FILE="${LOG_BASE_DIR}ALERT_${TASK}_${DATASET}_${EXPERIMENT_SETTING}_${candidate}_${COUNTERMEASURES}_${FACTOR}_${FPBP_DEL_PERCENTAGE}_tilde_beta_1_${run}_train.log"
    LOG_TEST_FILE="${LOG_BASE_DIR}ALERT_${TASK}_${DATASET}_${EXPERIMENT_SETTING}_${candidate}_${COUNTERMEASURES}_${FACTOR}_${FPBP_DEL_PERCENTAGE}_tilde_beta_1_${run}_test.log"
...
for FPBP_DEL_PERCENTAGE in "${FPBP_DEL_PERCENTAGES[@]}"
do
  LOG_BASE_FILE="${PROJECT_ROOT}/result/${DATASET}/${EXPERIMENT_SETTING}/ALERT_${TASK}_${DATASET}_${EXPERIMENT_SETTING}_${candidate}_${COUNTERMEASURES}_${FACTOR}_${FPBP_DEL_PERCENTAGE}_tilde_beta_1_"
...
for dataset in "${DATASETS[@]}"; do
    python ${PROJECT_ROOT}/baselines/test_attacks_default.py \
        --dataset "$dataset" \
        --test_times "$RUN_TIME" \
        --kws_uni_size "$FEATURE_NUM" \
        --kws_extraction "$KWS_EXTRACTION"

    python ${PROJECT_ROOT}/baselines/generate_test_attacks_default.py \
        --dataset "$dataset" \
        --test_times "$RUN_TIME" \
        --kws_uni_size "$FEATURE_NUM" \
        --kws_extraction "$KWS_EXTRACTION"

    python ${PROJECT_ROOT}/baselines/generate_result_default.py \
        --dataset "$dataset" \
        --kws_uni_size "$FEATURE_NUM" 
done
