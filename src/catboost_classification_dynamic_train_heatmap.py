...
def train_estimator(norm_input_dir1, input_dir1, norm_input_dir2, input_dir2, time_sel, delta, eta, if_pca, if_dcluster,if_savemodel, if_gpu, gpu_num,model_main_dir,model_pca_dir,dataset_name,heatmap_output_dir):

    time_select = time_sel
    ...
    if if_dcluster == True:
        ...
        while sign:
            for i in range(len(threshold_list)):
                ...
                # thres_mean = np.mean([thres_list[-1], thres_list[-1] + threshold])
                # print(avg_mean)
                # diff_thres = abs(thres_mean - avg_mean)
                # lowbound and upperbound to persure 95.4% data in the range of threshold
    ...
    for i in range(1):
        ...
        if if_pca == True:
            ...
            if if_savemodel == True:
                pca_model_path = f'{model_pca_dir}/pca_dcluster_{thres_list[i]}_{thres_list[i+1]}.pkl'
        ...
        # print(tran_reprocessed_matrix[i][0])
        # train_label = np.arange(select_number)
        ...
        y_final_proba = y_final_proba/random_number
        # print(y_final_proba)
        ...
        #     final_final_pred_label = final_label_temp[final_pred_label[l]]
        # final_final_pred_label = 
        ...
        prob_list = y_final_proba.tolist()
        pred_final_label_list = final_final_pred_label.tolist()
        pred_label_list = final_pred_label.tolist()
        true_label_list = final_label_ori.tolist()
        result_data = {
            "prob": prob_list,
            "pred_label": pred_label_list,
            "true_label": true_label_list,
            "pred_final_label": pred_final_label_list
        }
        # Save to a JSON file
        with open(heatmap_output_dir, "w") as file:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--if_pca', type=int, help='use or not use PCA',required=True,default=1)
    ...
    parser.add_argument('--if_gpu', type=int, help='save heatmap',required=True,default=1)
    ...
    parser.add_argument('--heatmap_output_dir', type=str, help='output dir',required=True,default='')
    ...
    train_estimator(args.norm_input_dir1, args.input_dir1, args.norm_input_dir2, args.input_dir2, args.time_sel,args.delta,args.eta,args.if_pca,args.if_dcluster,args.if_savemodel, args.if_gpu,args.gpu_num,args.model_main_dir,args.model_pca_dir,args.dataset_name,args.heatmap_output_dir)
